...
import { MenuItem, CartItem, OrderType, Order, ViewState } from './types';
...
const App: React.FC = () => {
      ...
      <div className="bg-background-dark text-white min-h-screen overflow-y-auto selection:bg-primary/30">
        <nav className="fixed top-0 w-full z-50 bg-black/40 backdrop-blur-md border-b border-white/5 py-4 md:py-6 px-6 md:px-12 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setView('landing')}>
            ...
            <span className="text-xl md:text-2xl font-black tracking-tighter">Golden Dragon</span>
          </div>

          <div className="hidden lg:flex items-center gap-10 text-xs font-bold uppercase tracking-widest text-gray-300">
            <a href="#" className="hover:text-primary transition-colors">Home</a>
            <button onClick={() => setView('menu')} className="hover:text-primary transition-colors">Menu</button>
          ...
          {isMobileMenuOpen && (
            <div className="lg:hidden fixed inset-0 top-[73px] bg-background-dark/95 z-50 p-8 flex flex-col gap-6 animate-in slide-in-from-top duration-300">
              <button
                ...
                className="text-2xl font-black text-left flex items-center justify-between group"
              >
                Menu <span className="material-icons-round text-primary group-hover:translate-x-2 transition-transform">east</span>
              </button>
              <a href="#" className="text-2xl font-black text-left" onClick={() => setIsMobileMenuOpen(false)}>About</a>
              <a href="#" className="text-2xl font-black text-left" onClick={() => setIsMobileMenuOpen(false)}>Contact</a>
              <div className="mt-auto pt-8 border-t border-white/10 space-y-4">
                <button onClick={() => setView('login')} className="w-full bg-white/5 p-5 rounded-2xl font-bold flex items-center justify-center gap-3">
                  <span className="material-icons-round">lock</span> Staff Login
                </button>
                <button onClick={() => setView('menu')} className="w-full bg-primary p-5 rounded-2xl font-black flex items-center justify-center gap-3">
                  Order Now <span className="material-icons-round">east</span>
                </button>
              </div>
            </div>
          )}
        ...
        <section className="relative h-[90vh] md:h-screen flex flex-col items-center justify-center text-center px-4 md:px-6">
          ...
          <div className="relative z-10 max-w-4xl space-y-6 md:space-y-8 animate-in fade-in zoom-in duration-1000">
            <div className="inline-flex items-center gap-2 bg-primary/10 border border-primary/20 px-3 md:px-4 py-1.5 md:py-2 rounded-full">
              ...
              <span className="text-primary text-[10px] md:text-xs font-black uppercase tracking-[0.2em]">Voted #1 Asian Cuisine</span>
            </div>

            <h1 className="text-4xl md:text-6xl lg:text-8xl font-black tracking-tighter leading-[0.95] md:leading-[0.9]">
              Authentic Chinese <br className="hidden md:block" />
              <span className="text-primary">At Your Door</span>
            </h1>

            <p className="text-gray-400 text-base md:text-lg lg:text-xl max-w-2xl mx-auto font-medium leading-relaxed px-4">
              Experience the taste of tradition with our family recipes passed down through generations. Premium ingredients, unforgettable flavors.
            </p>
        ...
        <section className="py-16 md:py-24 px-6 md:px-12 grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {[
            { icon: 'eco', title: 'Fresh Daily', desc: 'Locally sourced ingredients' },
            { icon: 'schedule', title: '30 Min Delivery', desc: 'Fast and hot to your door' },
            { icon: 'restaurant', title: 'Family Recipes', desc: 'Authentic flavors since 1985' },
          ].map((f, i) => (
            <div key={i} className="bg-white/5 border border-white/5 p-6 md:p-8 rounded-[2rem] md:rounded-3xl flex items-center gap-4 md:gap-6 group hover:bg-white/10 transition-all">
        ...
        <section className="py-16 md:py-24 px-6 md:px-12 space-y-12 md:space-y-16">
          <div className="text-center space-y-3 md:space-y-4">
            <h4 className="text-primary text-[10px] md:text-xs font-black uppercase tracking-[0.3em]">Our Menu Favorites</h4>
            <h2 className="text-3xl md:text-5xl font-black tracking-tight">Popular Dishes</h2>
            ...
            {MENU_ITEMS.slice(0, 4).map(item => (
              <div key={item.id} className="bg-surface-dark rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-5 border border-white/5 space-y-3 md:space-y-4 group">
                <div className="aspect-square rounded-2xl md:rounded-3xl overflow-hidden bg-black/40 relative">
                  ...
                  {item.isPopular && (
                    <div className="absolute top-4 right-4 bg-primary px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">
                      Spicy
                    </div>
                  )}
                </div>
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-primary font-black text-base md:text-lg">${item.price.toFixed(2)}</span>
                </div>
                <p className="text-gray-400 text-[10px] md:text-xs font-medium line-clamp-2">{item.description}</p>
                <button
                  onClick={() => {
                    setView('menu');
                    addToCart(item);
          ...
          <div className="text-center">
            <button onClick={() => setView('menu')} className="text-gray-400 hover:text-primary font-bold text-xs md:text-sm uppercase tracking-widest transition-colors inline-flex items-center gap-2">
              View Full Menu <span className="material-icons-round text-base">east</span>
            </button>
          </div>
        </section>
        ...
        <section className="bg-primary/90 py-16 md:py-24 px-6 md:px-12 mx-6 md:mx-12 rounded-[2rem] md:rounded-[3rem] mb-16 md:mb-24 flex flex-col md:flex-row items-center justify-between gap-10 md:gap-12 text-white relative overflow-hidden">
          <div className="relative z-10 space-y-6 text-center md:text-left">
            <h2 className="text-3xl md:text-5xl font-black tracking-tight">
              Order via our <br className="hidden md:block" /> Mobile App
            </h2>
            <p className="text-white/80 font-medium max-w-md mx-auto md:mx-0 text-sm md:text-base">
              Get exclusive deals and track your delivery in real-time. Available on iOS and Android.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
              <button className="bg-black text-white px-6 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl flex items-center justify-center gap-3 hover:scale-105 transition-all">
                ...
                <div className="text-left">
                  <p className="text-[8px] md:text-[10px] font-bold uppercase">Download on the</p>
              ...
              <button className="bg-black text-white px-6 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl flex items-center justify-center gap-3 hover:scale-105 transition-all">
                ...
                <div className="text-left">
                  <p className="text-[8px] md:text-[10px] font-bold uppercase">Get it on</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-12 mb-16">
            <div className="space-y-6">
              ...
              <p className="text-gray-500 text-sm font-medium leading-relaxed">
                Bringing authentic Chinese flavors to your table with a modern twist. Quality ingredients, exceptional taste.
              </p>
            </div>

            <div>
              <h4 className="font-black text-lg mb-6 md:mb-8">Opening Hours</h4>
              <div className="space-y-4 text-xs md:text-sm font-bold">
                <div className="flex justify-between text-gray-500"><span>Mon - Thu</span><span className="text-white">11:00 AM - 10:00 PM</span></div>
                <div className="flex justify-between text-gray-500"><span>Fri - Sat</span><span className="text-white">11:00 AM - 11:00 PM</span></div>
                <div className="flex justify-between text-gray-500"><span>Sunday</span><span className="text-white">12:00 PM - 10:00 PM</span></div>
              </div>
            </div>

            <div>
              <h4 className="font-black text-lg mb-6 md:mb-8">Contact Us</h4>
              <div className="space-y-4 text-xs md:text-sm font-bold text-gray-500">
                <div className="flex gap-3"><span className="material-icons-round text-primary text-lg">location_on</span><span>123 Dragon Way, Chinatown<br />New York, NY 10013</span></div>
            ...
            <div className="bg-white/5 rounded-2xl md:rounded-3xl p-2 h-40 md:h-48 overflow-hidden relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <button className="bg-white text-black px-4 py-2 rounded-lg text-[10px] md:text-xs font-black shadow-xl">Get Directions</button>
              </div>
            </div>
          </div>

          <div className="flex flex-col md:flex-row justify-between items-center pt-8 md:pt-12 border-t border-white/5 text-[8px] md:text-[10px] font-black uppercase tracking-widest text-gray-600 gap-4 text-center md:text-left">
            <p>© 2023 Golden Dragon Restaurant. All rights reserved.</p>
      ...
      <div className="bg-background-dark min-h-screen flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-surface-dark border border-white/10 rounded-[2.5rem] p-12 space-y-8 shadow-2xl">
          <div className="text-center space-y-2">
            ...
            <p className="text-gray-400 font-medium">Enter your password to access the dashboard</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest px-1">Password</label>
              <input
                type="password"
                value={loginPassword}
                ...
                autoFocus
              />
              {loginError && <p className="text-red-500 text-xs font-bold text-center animate-bounce">Incorrect password. Please try again.</p>}
            </div>
      ...
      <div className="bg-background-dark min-h-screen flex flex-col lg:flex-row text-white overflow-hidden">
        <header className="lg:hidden h-16 shrink-0 border-b border-white/5 bg-surface-dark px-4 flex items-center justify-between z-50">
          <div className="flex items-center gap-2" onClick={() => setView('landing')}>
            ...
            <span className="font-black tracking-tighter">Staff Panel</span>
          </div>
        ...
        <aside
          className={`
            ...
            ${isAdminSidebarOpen ? 'translate-x-0 pt-24 lg:pt-8' : '-translate-x-full lg:translate-x-0'}
          `}
        >
          <div className="hidden lg:flex items-center gap-2 cursor-pointer" onClick={() => setView('landing')}>
            ...
            <span className="text-2xl font-black tracking-tighter">Staff Panel</span>
          </div>
          ...
          <nav className="space-y-2">
            {[
              ...
              { id: 'menu', icon: 'restaurant_menu', label: 'Menu Editor' },
        ...
        <main className="flex-1 p-6 md:p-12 overflow-y-auto w-full">
          {adminView === 'dashboard' && (
            <div className="space-y-8 animate-in fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                  { label: 'Total Orders', value: todaysOrders.length, icon: 'receipt_long', color: 'bg-blue-500' },
                  { label: 'Total Revenue', value: `$${revenue.toFixed(2)}`, icon: 'attach_money', color: 'bg-green-500' },
                  { label: 'Pending Orders', value: pendingCount, icon: 'hourglass_top', color: 'bg-orange-500' },
                  { label: 'Occupied Tables', value: occupiedTables, icon: 'deck', color: 'bg-purple-500' },
                ].map((stat, i) => (
              ...
              <h3 className="text-xl font-bold mt-8 mb-4">Recent Orders</h3>
                ...
                {orders.slice(0, 5).map(o => (
                  <div key={o.id} className="bg-surface-dark border border-white/5 p-4 rounded-2xl flex justify-between items-center">
                    <div>
                      ...
                      <span className="ml-4 text-sm text-gray-400">{o.customerName || (o.tableNumber ? `Table ${o.tableNumber}` : 'Guest')}</span>
                    </div>
                    <span className="text-primary font-bold">${o.total.toFixed(2)}</span>
                  </div>
                ))}
                {orders.length === 0 && <p className="text-gray-500 text-sm">No recent orders.</p>}
              </div>
            </div>
          )}
          ...
          {adminView === 'orders' && (
            <div className="animate-in fade-in">
              <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8 md:mb-12">
                <div>
                  <h1 className="text-3xl md:text-4xl font-black tracking-tight">Active Orders</h1>
                  <p className="text-gray-400 font-medium mt-1 text-sm">Real-time status of current customer requests.</p>
                </div>
                <div className="flex gap-2">
                  {['All', 'New', 'Preparing', 'Ready', 'Completed'].map(f => (
                    <button key={f} onClick={() => setOrderFilter(f)} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${orderFilter === f ? 'bg-primary text-white' : 'bg-surface-dark text-gray-400 hover:text-white border border-white/5'}`}>
                      {f}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 gap-6">
                {filteredOrders.length === 0 ? (
                  <div className="bg-white/5 border border-dashed border-white/10 rounded-[3rem] h-96 flex flex-col items-center justify-center text-gray-500">
                    ...
                    <p className="font-bold text-xl">No active orders found.</p>
                  </div>
                ) : (
                  filteredOrders.map(o => (
                    <div key={o.id} className="bg-surface-dark border border-white/5 rounded-[2.5rem] p-6 md:p-8 flex flex-col xl:flex-row items-center gap-8 group hover:border-primary/20 transition-all">
                      <div className="flex items-center gap-6 shrink-0 w-full xl:w-auto">
                        <div className="w-16 h-16 bg-primary/10 rounded-2xl flex items-center justify-center text-primary font-black text-xl shrink-0">
                          {o.id.replace('#', '')}
                        </div>
                        <div className="flex-1">
                          ...
                          <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] flex gap-3">
                            ...
                            {o.tableNumber && <span className="text-primary">Table {o.tableNumber}</span>}
                          </p>
                      ...
                      <div className="w-full xl:w-auto shrink-0 flex flex-col md:flex-row xl:flex-col items-center justify-between xl:justify-center gap-4">
                        <p className="text-2xl font-black text-primary">${o.total.toFixed(2)}</p>
                        <div className="flex flex-wrap justify-center gap-2">
                          {o.status === 'pending' && <button onClick={() => updateOrderStatus(o.id, 'preparing')} className="bg-primary hover:bg-red-600 text-white px-4 py-2 rounded-xl font-bold transition-all text-xs">Accept & Prep</button>}
                          {o.status === 'preparing' && <button onClick={() => updateOrderStatus(o.id, 'ready')} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-xl font-bold transition-all text-xs">Mark Ready</button>}
                          {o.status === 'ready' && <button onClick={() => updateOrderStatus(o.id, 'completed')} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-xl font-bold transition-all text-xs">Complete</button>}
                          {o.status !== 'completed' && o.status !== 'cancelled' && <button onClick={() => updateOrderStatus(o.id, 'cancelled')} className="bg-white/5 hover:bg-red-500/10 text-gray-400 hover:text-red-500 px-4 py-2 rounded-xl font-bold transition-all text-xs">Cancel</button>}
          ...
          {adminView === 'menu' && (
            <div className="animate-in fade-in space-y-8">
              <div className="flex justify-between items-end">
                <h1 className="text-4xl font-black tracking-tight">Menu Editor</h1>
              ...
              {editingMenuItem && (
                <div className="bg-surface-dark border border-primary/30 rounded-[2rem] p-6 md:p-8 space-y-6">
                  <h3 className="text-2xl font-bold">{editingMenuItem.id ? 'Edit Item' : 'New Item'}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <input type="text" value={editingMenuItem.image || ''} onChange={e => handleMenuItemChange(e, 'image')} className="w-full bg-black/40 border-transparent rounded-xl py-3 px-4 text-sm" />
                    </div>
                    ...
                    <div className="md:col-span-2 flex items-center gap-3">
                      ...
                      <label htmlFor="isAvailable" className="text-sm font-bold text-gray-300">Item is available to order</label>
                    </div>
                  </div>
                  <div className="flex gap-4 pt-4 border-t border-white/10">
                    <button onClick={saveMenuItem} className="bg-green-500 hover:bg-green-600 text-white px-8 py-3 rounded-xl font-bold transition-all">Save</button>
                    <button onClick={() => setEditingMenuItem(null)} className="bg-white/10 hover:bg-white/20 text-white px-8 py-3 rounded-xl font-bold transition-all">Cancel</button>
                  </div>
                </div>
              )}
                  ...
                  <div key={item.id} className="bg-surface-dark border border-white/5 rounded-[2rem] p-4 flex gap-4 group hover:border-primary/20 transition-all">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-primary font-black mt-1 text-sm">${item.price.toFixed(2)}</p>
                      </div>
                      <div className="flex gap-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setEditingMenuItem(item)} className="bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg text-xs font-bold">Edit</button>
                        <button onClick={() => deleteMenuItem(item.id)} className="bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-all">Del</button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {adminView === 'settings' && (
            <div className="animate-in fade-in space-y-8 max-w-2xl">
              ...
              <div className="bg-surface-dark border border-white/5 rounded-[2.5rem] p-8 space-y-6">
                <h3 className="text-xl font-bold border-b border-white/10 pb-4">Restaurant Info</h3>
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Restaurant Name</label>
              ...
              <div className="bg-surface-dark border border-white/5 rounded-[2.5rem] p-8 space-y-6">
                <h3 className="text-xl font-bold border-b border-white/10 pb-4">Fees & Taxes</h3>
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">rate</span>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Service Fee ($)</label>
              ...
              <div className="bg-surface-dark border border-white/5 rounded-[2.5rem] p-8 text-center text-gray-400">
                <span className="material-icons-round text-5xl mb-4 text-primary opacity-50">qr_code_2</span>
                <h3 className="text-xl font-bold text-white mb-2">QR Code Generator</h3>
                <p className="text-sm">Connect a backend to generate table-specific QR codes for Dine-In.</p>
              </div>
            </div>
          )}
          ...
          {adminView === 'analytics' && (
            <div className="h-full flex flex-col items-center justify-center text-gray-500 opacity-50">
              <span className="material-icons-round text-6xl mb-4">insights</span>
              <p className="font-bold text-xl">Analytics Dashboard Coming Soon</p>
            </div>
          )}
        </main>
      </div>
  ...
  if (view === 'success') {
    ...
    return (
      <div className="h-screen bg-background-dark flex flex-col items-center justify-center p-8 text-center text-white">
        ...
        <p className="text-gray-400 max-w-md text-lg mb-8">Your order from Golden Dragon is confirmed and will be ready soon.</p>
          ...
          <div className="bg-surface-dark border border-white/10 rounded-3xl p-6 md:p-8 w-full max-w-sm mb-12 space-y-4 text-left shadow-2xl">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold flex items-center gap-2">
                ...
                {lastOrder.type === OrderType.DINE_IN ? 'Dine-In' : lastOrder.type === OrderType.DELIVERY ? 'Delivery' : 'Collection'}
              </span>
            </div>
            {lastOrder.tableNumber && (
               <div className="flex justify-between items-center bg-primary/10 p-3 rounded-xl mt-2">
                 <span className="text-primary font-bold uppercase tracking-widest text-xs">Table</span>
            ...
            <div className="flex justify-between items-center pt-2">
              ...
              <span className="text-yellow-500 font-bold bg-yellow-500/10 px-3 py-1 rounded-full text-xs">Pending</span>
            </div>
          </div>
        )}
  ...
  const categories = ['All Menu', ...Array.from(new Set(menuItems.map(item => item.category)))];
  const filteredItems = activeCategory === 'All Menu' ? menuItems : menuItems.filter(item => item.category === activeCategory);
    ...
    <div className="bg-background-light dark:bg-background-dark font-display text-gray-800 dark:text-gray-100 h-screen overflow-hidden flex flex-col">
      <header className="h-16 md:h-20 shrink-0 border-b border-gray-200 dark:border-white/10 bg-white dark:bg-surface-dark px-4 md:px-6 flex items-center justify-between z-20 shadow-md">
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <div className="flex items-center gap-2 md:gap-3 cursor-pointer" onClick={() => setView('landing')}>
            ...
            <div className="hidden xs:block">
              <h1 className="font-bold text-base md:text-xl tracking-tight leading-none">Golden Dragon</h1>
              <p className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400">Authentic Chinese Cuisine</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-xl mx-4 md:mx-8">
          <div className="relative">
            <span className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-gray-400 material-icons-round text-sm md:text-base">search</span>
            <input className="w-full bg-gray-100 dark:bg-black/20 border-transparent rounded-xl py-2 md:py-3 pl-9 md:pl-12 pr-4 text-xs md:text-sm" placeholder="Search dishes..." type="text" />
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
          ...
          {view === 'menu' ? (
            <>
              ...
              <div className="flex-1 overflow-y-auto px-4 md:px-8 pb-32">
                <h2 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">{activeCategory}</h2>
                  ...
                  {filteredItems.map(item => (
                    <div key={item.id} className="group bg-white dark:bg-card-dark rounded-2xl md:rounded-3xl p-3 md:p-4 hover:shadow-2xl transition-all border dark:border-white/5 flex flex-col h-full relative overflow-hidden">
                      <div className="aspect-square rounded-xl md:rounded-2xl overflow-hidden mb-3 md:mb-4 bg-gray-800 relative">
                        <img className={`w-full h-full object-cover transition-transform duration-700 ${item.isAvailable === false ? 'opacity-50 grayscale' : 'group-hover:scale-110'}`} src={item.image} alt={item.name} />
                        {item.isAvailable === false ? (
                          <div className="absolute inset-x-0 bottom-0 top-0 flex items-center justify-center bg-black/40 backdrop-blur-[2px]">
                            <span className="bg-red-500/90 text-white font-black px-4 py-2 text-xs rounded-xl uppercase tracking-widest border border-red-400">Sold Out</span>
                          </div>
                        ) : (
                      ...
                      <div className="flex-1">
                        <h3 className="font-bold text-base md:text-lg mb-0.5 md:mb-1">{item.name}</h3>
                        <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-2 md:mb-4 line-clamp-2">{item.description}</p>
                        <span className="font-bold text-lg md:text-2xl text-primary">${item.price.toFixed(2)}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex-1 p-4 md:p-8 overflow-y-auto w-full max-w-2xl mx-auto">
              <h2 className="text-2xl md:text-4xl font-bold mb-6 md:mb-8 text-center md:text-left">Checkout</h2>
              <div className="space-y-6 bg-white dark:bg-card-dark p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-gray-100 dark:border-white/5 shadow-2xl">
                ...
                {tableNumber && (
                   <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 flex items-center justify-between">
                     <div>
                       <p className="text-primary font-black uppercase text-xs tracking-widest mb-1">Dine-In Order</p>
                       <p className="font-bold">Ordering for Table {tableNumber}</p>
                     </div>
                     <span className="material-icons-round text-primary text-3xl">restaurant</span>
                   </div>
                )}
                ...
                <div className="space-y-4 md:space-y-6">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2 px-1 tracking-widest">Full Name {orderType === OrderType.DINE_IN && '(Optional)'}</label>
                    <input type="text" value={userDetails.name} onChange={e => setUserDetails({ ...userDetails, name: e.target.value })} className="w-full bg-gray-50 dark:bg-black/30 border-transparent rounded-2xl py-3 md:py-4 px-5 md:px-6 focus:ring-primary focus:border-primary transition-all text-sm md:text-base" placeholder="Enter your name" />
                  </div>

                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2 px-1 tracking-widest">Phone Number {orderType === OrderType.DINE_IN && '(Optional)'}</label>
                    <input type="tel" value={userDetails.phone} onChange={e => setUserDetails({ ...userDetails, phone: e.target.value })} className="w-full bg-gray-50 dark:bg-black/30 border-transparent rounded-2xl py-3 md:py-4 px-5 md:px-6 focus:ring-primary focus:border-primary transition-all text-sm md:text-base" placeholder="Enter phone number" />
                  </div>

                  {orderType === OrderType.DELIVERY && (
                    <div className="animate-in fade-in slide-in-from-top-4">
                      <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2 px-1 tracking-widest">Delivery Address</label>
                      <textarea rows={2} value={userDetails.address} onChange={e => setUserDetails({ ...userDetails, address: e.target.value })} className="w-full bg-gray-50 dark:bg-black/30 border-transparent rounded-2xl py-3 md:py-4 px-5 md:px-6 focus:ring-primary focus:border-primary transition-all text-sm md:text-base" placeholder="Street, Apartment, Postcode" />
                    </div>
                  )}

                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2 px-1 tracking-widest">Order Notes (Optional)</label>
                    <textarea rows={2} value={orderNotes} onChange={e => setOrderNotes(e.target.value)} className="w-full bg-gray-50 dark:bg-black/30 border-transparent rounded-2xl py-3 md:py-4 px-5 md:px-6 focus:ring-primary focus:border-primary transition-all text-sm md:text-base" placeholder="Any allergies or special requests?" />
                  </div>
                </div>
                ...
                <div className="pt-4 md:pt-6 border-t border-gray-100 dark:border-white/5">
                  ...
                  <button onClick={() => setView('menu')} className="w-full mt-4 md:mt-6 text-xs md:text-sm text-gray-500 font-medium hover:text-primary transition-colors flex items-center justify-center gap-2">
                    <span className="material-icons-round text-sm">arrow_back</span> Return to Menu
                  </button>
                </div>
              </div>
            </div>
        ...
        <aside
          className={`
            ...
            flex flex-col shrink-0 shadow-[-10px_0px_30px_rgba(0,0,0,0.1)]
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 md:space-y-8">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64 text-gray-400 space-y-4 opacity-50">
                ...
                <p className="text-sm text-center">Tap dishes to add them to your basket</p>
              </div>
            ) : (
              cart.map(item => (
                <div key={item.cartId} className="flex gap-4 md:gap-5 group animate-in slide-in-from-right-4">
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-bold text-sm md:text-base leading-tight pr-2">{item.name}</h4>
                      <span className="font-black text-sm md:text-base">${(item.price * item.quantity).toFixed(2)}</span>
                    </div>
          ...
          <div className="p-6 md:p-8 bg-gray-50 dark:bg-card-dark border-t dark:border-white/10">
            {!tableNumber ? (
              <div className="flex gap-3 mb-6 md:mb-8">
                <button onClick={() => setOrderType(OrderType.COLLECTION)} className={`flex-1 flex flex-col items-center justify-center p-3 md:p-5 rounded-[1.2rem] md:rounded-[1.5rem] border-2 transition-all ${orderType === OrderType.COLLECTION ? 'border-primary bg-primary/5 shadow-inner' : 'border-gray-100 dark:border-white/5 bg-white dark:bg-surface-dark'}`}>
                  ...
                  <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest">Collection</span>
                </button>
                <button onClick={() => setOrderType(OrderType.DELIVERY)} className={`flex-1 flex flex-col items-center justify-center p-3 md:p-5 rounded-[1.2rem] md:rounded-[1.5rem] border-2 transition-all ${orderType === OrderType.DELIVERY ? 'border-primary bg-primary/5 shadow-inner' : 'border-gray-100 dark:border-white/5 bg-white dark:bg-surface-dark'}`}>
                  ...
                  <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest">Delivery</span>
                </button>
              </div>
            ) : (
            ...
            <div className="space-y-2 md:space-y-3 mb-6 md:mb-8">
              <div className="flex justify-between text-xs md:text-sm text-gray-500 font-bold"><span>Subtotal</span><span>${subtotal.toFixed(2)}</span></div>
              <div className="flex justify-between text-xs md:text-sm text-gray-500 font-bold"><span>Tax & Service</span><span>${(tax + (subtotal > 0 ? SERVICE_FEE : 0)).toFixed(2)}</span></div>
